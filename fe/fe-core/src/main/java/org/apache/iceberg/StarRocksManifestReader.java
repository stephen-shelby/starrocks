 ...
 */
public class StarRocksManifestReader<F extends ContentFile<F>> extends CloseableGroup
    ...
    protected StarRocksManifestReader(
            InputFile file,
            int specId,
    ...
    public boolean isDeleteStarRocksManifestReader() {
    ...
    public StarRocksManifestReader<F> select(Collection<String> newColumns) {
    ...
    public StarRocksManifestReader<F> project(Schema newFileProjection) {
        Preconditions.checkState(
    ...
    public StarRocksManifestReader<F> filterPartitions(Expression expr) {
        this.partFilter = Expressions.and(partFilter, expr);
        return this;
    }

    public StarRocksManifestReader<F> filterPartitions(PartitionSet partitions) {
        this.partitionSet = partitions;
        return this;
    }

    public StarRocksManifestReader<F> filterRows(Expression expr) {
        this.rowFilter = Expressions.and(rowFilter, expr);
        return this;
    }

    public StarRocksManifestReader<F> caseSensitive(boolean isCaseSensitive) {
        this.caseSensitive = isCaseSensitive;
        return this;
    }

    StarRocksManifestReader<F> scanMetrics(ScanMetrics newScanMetrics) {
    ...
    private static boolean requireStatsProjection(Expression rowFilter, Collection<String> columns) {
        ...
        return rowFilter != Expressions.alwaysTrue()
                && columns != null
                && !columns.containsAll(StarRocksManifestReader.ALL_COLUMNS)
    ...
    static boolean dropStats(Collection<String> columns) {
        ...
        // record_count, which
        // is a primitive type.
        if (columns != null && !columns.containsAll(StarRocksManifestReader.ALL_COLUMNS)) {
    ...
    static List<String> withStatsColumns(Collection<String> columns) {
        if (columns.containsAll(StarRocksManifestReader.ALL_COLUMNS)) {
